...
class ResultsPanel(QWidget):
    ...
    def __init__(self, parent=None) -> None:
        ...
        self.text.setPlaceholderText("No ROIs analyzed yet.\n\n"
                                      "1. Open a DICOM folder\n"
                                      "2. Pick a slice with the slider\n"
                                      "3. Click 'Polygon: OFF' to enable drawing\n"
                                      "4. Left-click to add vertices, double-click to close\n"
                                      "5. Click 'Analyze' to compute metrics")
    ...
    def show_result(self, name: str, result: Dict[str, Any]) -> None:
        """Display metrics for one ROI in the text pane + histogram."""
        self._current_result = result
        self.text.setPlainText(_format_metrics(name, result))
        self._draw_histogram(result)

    def show_all(self, results: Dict[str, Dict[str, Any]]) -> None:
        """Display a summary table of all analyzed ROIs."""
        if not results:
            self.clear()
            return
        ...
        # Pick the first ROI's histogram
    ...
    def _draw_histogram(self, result: Dict[str, Any]) -> None:
        ...
        # Overlay clinical threshold lines (if present)
        ...
        self._ax.set_title(f"Histogram — {result.get('name', '?')} ({hr.n_voxels} voxels)")
        self._ax.set_xlabel("HU")
        self._ax.set_ylabel("Voxel count")
...
def _format_metrics(name: str, r: Dict[str, Any]) -> str:
    """Format a single ROI's metrics for the text pane."""
    lines = [
    ...
    if pm:
        lines.append("")
        lines.append("  Psoas / myosteatosis:")
        ...
        lines.append(f"    Myosteatosis flag : {pm.get('myosteatosis_flag', False)}")
    return "\n".join(lines)
